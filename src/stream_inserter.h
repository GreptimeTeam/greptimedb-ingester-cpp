...
/// When writing or WriteBatch is called, only the Request is written to the
/// Buffer. To prevent OOM, the Buffer has a capacity limit determined by
/// BUFFER_SIZE A background thread periodically obtains a batch of Requests
/// from the Buffer and sends a batch through the gRPC. The number of requests
/// in a batch is determined by BATCH_BYTE_LIMIT
class StreamInserter {
 public:
  StreamInserter(std::string dbname_, std::shared_ptr<Channel> channel_,
  ...
  ~StreamInserter() { delete scheduler_thread; }
  ...
  /// sends a GreptimeRequest consisted of a batch of InsertRequests through the
  /// gRPC.
  bool Send(const GreptimeRequest &greptime_request);

  /// A background thread periodically obtains a batch of Requests from the
  /// Buffer and sends a batch through the gRPC.
