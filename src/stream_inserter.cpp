...
void StreamInserter::Write(InsertRequest insert_request) {
  ...
  cv.wait(lk, [this] { return this->buffer.size() + 1 < BUFFER_SIZE; });
...
void StreamInserter::WriteBatch(std::vector<InsertRequest> insert_request_vec) {
  ...
  std::unique_lock<std::mutex> lk(mtx);
  cv.wait(lk, [this, cnt] { return this->buffer.size() + cnt < BUFFER_SIZE; });
  ...
  while (true) {
    ...
    if (!buffer.empty()) {
      ...
      if (!Send(greptime_request)) {
        std::cout << "Greptime Request to large: "
                  << greptime_request.ByteSizeLong() << std::endl;
      }
    }
  }
}
